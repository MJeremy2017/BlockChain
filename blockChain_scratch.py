...
genesisBlockContents = {u'blockNumber': 0, u'parentHash': None, u'txnCount': 1,
                        u'txns': genesisBlockTxns}  # convert to a hash code
...
# 30 transactions in txnBuffer
while len(txnBuffer) > 0:
    bufferStartSize = len(txnBuffer)

    # Gather a set of valid transactions for inclusion
    txnList = []
    while (len(txnBuffer) > 0) & (len(txnList) < blockSizeLimit):  # multiple txn in sync
        newTxn = txnBuffer.pop()  # pop the last item of list
...
chain[0]  # genesis block with initial balance of accounts
chain[2]


# just to check if the current hash match current block
...
# state is global, only transactions are stored in blocks
try:
